...
export default function LoginModal() {
  ...
  const handleLogin = async (e: FormEvent) => {
    ...
    if (!result.success) {
      ...
      setMessage(result.message || t("auth:auth.login.loginFailed"))
    }
  }
  ...
  return (
    ...
    >
      <DialogContent className="sm:max-w-[425px] bg-gray-900 border-yellow-500/30 text-gray-100 max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-2xl font-bold text-yellow-400">{t("auth:auth.login.title")}</DialogTitle>
          <DialogDescription className="text-gray-400">{t("auth:auth.login.subtitle")}</DialogDescription>
        </DialogHeader>

        {/* Demo accounts info */}
        <div className="bg-blue-500/10 border border-blue-500/20 rounded-lg p-3 mb-4">
          <p className="text-blue-400 text-sm font-medium mb-2">{t("auth:auth.login.demoAccounts")}</p>
          <div className="text-xs text-blue-300 space-y-1">
            <div>{t("auth:auth.login.demoRegular")}</div>
            <div>{t("auth:auth.login.demoPremium")}</div>
          </div>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-4 py-4">
          ...
          <div>
            <Label htmlFor="email-modal" className="flex items-center space-x-2 text-yellow-400 mb-2">
              ...
              <span>{t("auth:auth.login.email")}</span>
            </Label>
            <Input
              id="email-modal"
              type="email"
          ...
          <div>
            <Label htmlFor="password-modal" className="flex items-center space-x-2 text-yellow-400 mb-2">
              ...
              <span>{t("auth:auth.login.password")}</span>
            </Label>
          ...
          <div className="flex items-center justify-between">
            ...
            >
              {t("auth:auth.login.forgotPassword")}
            </button>
          </div>
