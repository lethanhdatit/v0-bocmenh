...
const tourSteps: TourStep[] = [
  { id: "hero-section", titleKey: "tour.hero.title", descriptionKey: "tour.hero.description" },
  { id: "features-section", titleKey: "tour.features.title", descriptionKey: "tour.features.description" },
  { id: "luckybox-section", titleKey: "tour.luckybox.title", descriptionKey: "tour.luckybox.description" },
  {
        ...
        {isTourActive && showTourTooltip && (
          ...
          >
            <div className="mystical-card max-w-md w-full p-6 relative text-center">
              ...
              <h3 className="text-xl font-semibold text-yellow-400 mb-2">{tooltipContent.title}</h3>
              <p className="text-gray-300 text-sm mb-4">{tooltipContent.description}</p>
              <div className="text-xs text-gray-500">
                {t("tour.autoNext", { count: 5 - (currentTourStep + 1) * 0 })} {/* Logic count cần review */}
              </div>
              <div className="flex justify-center mt-4 space-x-3">
                ...
                {currentTourStep < tourSteps.length - 1 && (
                  <Button
                    onClick={() => {
                      if (currentTimeoutIdRef.current) clearTimeout(currentTimeoutIdRef.current)
                ...
                {tourSteps.map((_, index) => (
                  <div
                    ...
                    className={`w-2.5 h-2.5 rounded-full transition-all duration-300 ${
                      index === currentTourStep ? "bg-yellow-500 scale-125" : "bg-gray-600"
                    }`}
                  />
                ))}
              </div>
