...
import { Stars, Calendar, Clock, User, Building, Loader2, AlertCircle } from "lucide-react"
...
export default function DestinyForm() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      setFormError(error.response?.data?.message ?? error.response?.errors?.general ?? t("common.errorUnexpected"))
    } finally {
      setIsLoading(false)
    }
  }
  ...
  return (
    <>
      <GlobalLoading isVisible={isLoading} message={t("destiny.form.loadingMessagePage")} />
      ...
      <Card className="w-full max-w-lg mx-auto bg-gray-900/80 border-yellow-500/30 shadow-xl backdrop-blur-sm">
        ...
        <CardContent>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              <Label htmlFor="birthPlace" className="flex items-center space-x-2 text-yellow-400 font-medium mb-1.5">
