...
function TruCell({
  ...
  if (field === "napAm") return <td className={className}>{tru.napAm.display}</td>;
  return <td className={className}>--</td>;
}

function TruAmDuong({ tru, field }: { tru?: PreDataTru; field: "can" | "chi" }) {
  if (!tru) return <td>--</td>;
  return <td className="text-xs text-gray-500">{tru[field].amDuong.display}</td>;
}

function TruNguHanh({ tru, field }: { tru?: PreDataTru; field: "can" | "chi" }) {
  if (!tru) return <td>--</td>;
  return <td className="text-xs text-gray-500">{tru[field].nguHanhBanMenh.display}</td>;
}
...
function TruVongTruongSinh({ tru }: { tru?: PreDataTru }) {
  ...
  return <td className="text-xs text-gray-500">{tru.vongTruongSinh.join(", ")}</td>;
}
...
export function LaSoBatTuSection({ preData, t }: { preData: PreDataModel; t: any }) {
  return (
    <section className="p-6 bg-gray-900/80 rounded-xl border border-yellow-500/30 shadow-lg">
      ...
      <div className="overflow-x-auto">
        <table className="mx-auto border border-yellow-400 rounded-lg bg-yellow-50 text-center min-w-[900px]">
          <thead>
          ...
          <tbody className="text-gray-900">
            ...
            <tr>
              ...
              {COLS.map((col) => (
                <TruAmDuong key={col.key} tru={getTru(preData, col.key)} field="can" />
              ))}
            </tr>
            {/* Ngũ hành Can */}
            <tr>
              ...
              {COLS.map((col) => (
                <TruNguHanh key={col.key} tru={getTru(preData, col.key)} field="can" />
              ))}
            </tr>
            {/* Địa chi */}
            <tr>
            ...
            <tr>
              ...
              {COLS.map((col) => (
                <TruAmDuong key={col.key} tru={getTru(preData, col.key)} field="chi" />
              ))}
            </tr>
            {/* Ngũ hành Chi */}
            <tr>
              ...
              {COLS.map((col) => (
                <TruNguHanh key={col.key} tru={getTru(preData, col.key)} field="chi" />
              ))}
            </tr>
            {/* Thập thần */}
            <tr>
            ...
            <tr>
              ...
              {COLS.map((col) => (
                <TruCell key={col.key} tru={getTru(preData, col.key)} field="napAm" />
              ))}
            </tr>
            {/* Trường sinh */}
            <tr>
              ...
              {COLS.map((col) => (
                <TruVongTruongSinh key={col.key} tru={getTru(preData, col.key)} />
              ))}
            </tr>
            {/* Thần sát */}
            <tr>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-800 mt-6">
        <div>
          <div>
