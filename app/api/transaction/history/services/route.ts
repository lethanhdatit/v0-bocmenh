...
async function getServiceHistoryHandler(
  _data: any,
  request: NextRequest,
  ...
  try {
    ...
    const response = await apiServer.get(
      `/transaction/services/histories?pageSize=${pageSize}&pageNumber=${pageNumber}`,
      {
        ...config,
      }
    );
    ...
    return handleApiServerError(
      error,
      {
        error400Message: t("topups.error.getServiceHistoryFailed"),
...
export const GET = withServerBase(getServiceHistoryHandler, {
